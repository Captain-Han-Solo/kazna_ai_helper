...
Процедура ПроцессИтогиЗапроса(РезультатЗапроса)
	...
	ПроверитьОтветЭкспертаИСообщитьНаСервере(РезультатЗапроса.ТекстОтвета);
...
Функция СформироватьСтруктураПромпта(Знач Эксперт, Знач Промпт)
	...
	ТекстФормата = КИИ_ЭкспертыИИ.ТекстФорматаОтветаДляПромпта(Эксперт);
...
&НаСервере
Процедура ПроверитьОтветЭкспертаИСообщитьНаСервере(Знач ТекстОтвета)

	СхемаОтвета = КИИ_ЭкспертыИИ.ПолучитьСхемуОтветаЭксперта(Эксперт);

	Если Не ЗначениеЗаполнено(СхемаОтвета) Тогда
		Возврат;
	КонецЕсли;

	Итог = КИИ_ЭкспертыИИ.ПроверитьОтветПоСхеме(Эксперт, ТекстОтвета);
	КИИ_ВалидацияОтветаИИ.СообщитьИтогВалидацииПользователю(Итог);

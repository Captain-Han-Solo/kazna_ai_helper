...
//  Документ - ДокументСсылка, Неопределено - документ-инициатор запроса; попадает в лог запросов ИИ
//  НомерПовтора - Число - порядковый номер запроса к модели в рамках одной попытки анализа (>= 1)
//
...
//  КонецЕсли;
//
Функция ЗапросКМодели(Модель, Промпт, Документ = Неопределено, НомерПовтора = 1) Экспорт
	...
	ЗаписатьЛогИИ(Модель, СтруктураПромпта, Результат, Документ, НомерПовтора);
...
Процедура ЗаписатьЛогИИ(Модель, СтруктураПромпта, Результат, Документ = Неопределено, НомерПовтора = 1)
	...
	МенеджерЗаписи.НомерПовтора = ?(НомерПовтора > 0, НомерПовтора, 1);

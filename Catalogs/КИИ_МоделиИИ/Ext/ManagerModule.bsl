...
// Возвращает признак поддержки Constrained Decoding по JSON Schema для модели.
//
// Параметры:
...
//  Булево
//
Функция ПоддерживаетConstrainedDecoding(Модель) Экспорт

	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Модель, "ПоддерживаетConstrainedDecoding");
...
Процедура ЗаполнитьРеквизитыМодели(МодельОбъект, ДанныеМодели)
	...
	Если ДанныеМодели.Свойство("ПоддерживаетConstrainedDecoding") Тогда
		МодельОбъект.ПоддерживаетConstrainedDecoding = ДанныеМодели.ПоддерживаетConstrainedDecoding;
	КонецЕсли;
